...
export function PostForm({
  ...
  const validateScheduledDate = (value: string) => {
    if (isScheduled && (!value || value.trim() === "")) {
  ...
  const validatePrice = (value: number) => {
    ...
    if (currentVisibility === "pay-to-view") {
      if (!value || value < 0.01) return "Price must be at least $0.01";
      if (value > 999.99) return "Price must be less than $999.99";
    }
    return undefined;
  };
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      if (
        value.visibility === "pay-to-view" &&
        (!value.price || Number(value.price) < 0.01)
      ) {
      ...
      if (
        isScheduled &&
        (!value.scheduledDate || value.scheduledDate.trim() === "")
      ) {
        toast.error("Scheduled date is required");
        return;
      }

      const mediaFileIds = uploadedMediaFiles
        .filter((m) => m.id && !m.url.startsWith("blob:"))
        .map((m) => m.id);
  ...
  const addMediaFromGallery = (selectedMedia: MediaFile[]) => {
    ...
    const validMedia = selectedMedia.filter(
      (media) => !isBlobUrl(media.url || "")
    );
  ...
  const handleUploadComplete = (results: any[]) => {
    ...
    const filtered = uploadedMediaFiles.filter(
  ...
  const getSubmitButtonText = () => {
    if (isEditMode) {
      return isScheduled
        ? t("updateSchedule") || "Update Schedule"
        : t("postForm.updatePost") || "Update Post";
    } else {
      return isScheduled
  ...
  const isFormValid = useCallback((): boolean => {
    ...
    const content = form.state.values.content?.trim() || "";
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between text-sm text-muted-foreground">
                    <span>
                      ...
                      {uploadedMediaFiles.length === 1 ? "file" : "files"}{" "}
                      uploaded
                    </span>
                    ...
                    <div className="mt-2 space-y-3">
                      ...
                      <div className="flex items-start">
                        ...
                        <div className="ml-3 text-sm">
                          ...
                          <p className="text-muted-foreground">
                            {t("subscribersOnlyHint") ||
                              "Only visible to your paid subscribers"}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-start">
                        ...
                        <div className="ml-3 text-sm">
                          ...
                          <p className="text-muted-foreground">
                            {t("payPerViewHint") ||
                              "Users must pay a one-time fee to access"}
                          </p>
                        </div>
                      </div>
                    </div>
            ...
            <div className="md:col-span-2">
              <div className="flex items-start">
                ...
                <div className="ml-3 text-sm">
                  ...
                  <p className="text-muted-foreground">
                    {t("scheduleHint") ||
                      "Set a future date and time to publish this post"}
                  </p>
                </div>
              </div>
